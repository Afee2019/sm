...
class mergeImgs(wx.Frame):
    def __init__(
        ...
        #===============================================
        #
        l2 = wx.StaticText(panel, -1, "目标数据源")
        ...
        #===============================================
        #
        l3 = wx.StaticText(panel, -1, "目标数据集")
        ...
        #
        pos= 15
        l4 = wx.StaticText(panel, -1, "数据集类型")
        ...
        pos = 15
        ypos = 220
        l8 = wx.StaticText(panel, -1, "水平分辨率")
        ...
        pos+= 280
        l9 = wx.StaticText(panel, -1, "竖直分辨率")
        ...
        pos = 15
        ypos = 260
        l8 = wx.StaticText(panel, -1, "宽度(像素)")
        l8.SetPosition((pos,ypos))
        ...
        pos+= 280
        l9 = wx.StaticText(panel, -1, "高度(像素)")
        l9.SetPosition((pos,ypos))
        ...
        ypos = 300
        btnWidth = 150
        btnGetBounds = wx.Button(panel, -1, "获取范围和分辨率",pos = (xpos, ypos),size=(btnWidth,-1))
        self.Bind(wx.EVT_BUTTON, self.OnGetInfo, btnGetBounds)

   
        xpos+=170
        btnMerge = wx.Button(panel, -1, "拼接影像",pos = (xpos, ypos),size=(btnWidth,-1))
        self.Bind(wx.EVT_BUTTON, self.OnMerge, btnMerge)

        xpos+=170
        btnClose = wx.Button(panel, -1, "放弃操作",size=(btnWidth,-1))
    ...
    def OnGetInfo(self, event):
        if len(self.t1.Value)<1:
            dlg = wx.MessageDialog(self, '请选择文件夹',
                               '请选择文件夹',
        ...
        if len(self.c12.Value)<1:
            dlg = wx.MessageDialog(self, '请选择文件类型',
                               '请选择文件类型',
                               wx.OK | wx.ICON_INFORMATION
                               #wx.YES_NO | wx.NO_DEFAULT | wx.CANCEL | wx.ICON_INFORMATION
                               )
        ...
        for file in self.fl.FileLists:
            info = smu.GetImgFileInfo(file, self.c12.Value)
            if bInit:
    ...
    def OnSelFolder(self, event):
        folder = tk.ChooseFolder("选择文件夹")
        self.t1.Value = folder

    def OnSelFile(self, event):
        folder = tk.ChooseSaveFile("目标数据源",'UDB数据源(*.udb)|*.udb')
